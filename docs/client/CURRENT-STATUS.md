...
- 项目背景：[PROJECT-CONTEXT.md](PROJECT-CONTEXT.md)
- 协作规则：[CLIENT-COLLAB-RULES.md](CLIENT-COLLAB-RULES.md)
- 当前执行真相：[CURRENT-SOURCE-OF-TRUTH.md](00-runtime/CURRENT-SOURCE-OF-TRUTH.md)
- 主界面改版联调：[Main 首页 P0 结构整改执行单（当前版）.md](06-开发规范/Main%20%E9%A6%96%E9%A1%B5%20P0%20%E7%BB%93%E6%9E%84%E6%95%B4%E6%94%B9%E6%89%A7%E8%A1%8C%E5%8D%95%EF%BC%88%E5%BD%93%E5%89%8D%E7%89%88%EF%BC%89.md)
- 冻结总决策：[DECISION-RESULT-FROZEN-V1.0.md](05-决策记录/DECISION-RESULT-FROZEN-V1.0.md)
- 客户端联调验收：[客户端联调验收清单-V1.0.md](06-开发规范/客户端联调验收清单-V1.0.md)

## 2026-04-20 里程碑

- 学生端主界面壳子已开始转向 viewport-safe 的响应式布局基础。
- Runtime UI primitives 已升级，可用于 card / badge / progress / action tile / speech bubble / mini input。
- 学生端顶栏已调整为响应式组合壳子，包含 pill tabs 和紧凑控制。
- 紧凑学生概览现在隐藏常驻右侧事件面板，不再保留旧的固定三栏观感。
- Main 学生概览面板和操作栏现在根据 layout metrics 推导尺寸与位置，不再只使用固定坐标。
- Main 学生场景面板和紧凑事件面板现在根据 layout metrics 推导标题、卡片与摘要间距。
- `PetGrowthView` 已对齐同一套暖色响应式卡片系统，不再像旧暗色主题残留页面。
- `MainController.renderGlobalActions` 内旧的学生分支已移除，旧 header action shell 不再与新的学生顶栏共存。
